...
export function RelicSearch() {
  ...
  useEffect(() => {
    ...
    if (selectedParts.length > 0) {
      searchRelicsForSelected();
    }
    else {
      setRelics([]);
    }
  }, [searchTerm, selectedParts]);
  ...
  const searchRelicsForSelected = async () => {
    const searchResponse = await fetch(`/api/relic/search`, {method: 'POST', body: JSON.stringify(selectedParts)});
    const data = await searchResponse.json();
    setRelics(data);
  }
